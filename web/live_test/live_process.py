...
class Process(object):
    def __init__(self, metadata):
        # change!!
        self.model = tf.keras.models.load_model('live_test/model/ver4-6_f(el,er)h_mobile.h5')  # load model
    ...
    def process_img(self, input):
        ...
        focus = 0
        if len(self.label_list) == 10:  # check focus every 30 sec
            focus = self.check_focus()

        # make metadata for professor_page
        self.meta.sum += label
        self.meta.minute_sum += label
        if (self.meta.cnt % 60 == 0):
            self.meta.minute_sum = 0
            self.meta.minute_cnt=1
        meta_dic = {'time': input_time, 'value': label, 'data_rate': round(self.meta.sum / self.meta.cnt, 2),
                    'minute_rate': round(self.meta.minute_sum / self.meta.minute_cnt, 2)}
        self.meta.student_info.append(meta_dic)
        self.meta.cnt += 1
        self.meta.minute_cnt+=1
        if meta_dic['value'] == 0:
            output_img = cv2.rectangle(original, (10, 10), (310, 310), (0, 0, 255), -1)

        elif meta_dic['value'] == 1:
            output_img = cv2.rectangle(original, (10, 10), (310, 310), (255, 0, 0), -1)
